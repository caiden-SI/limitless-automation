...
//   7. handle() returns { recovered: boolean } so callers can decide whether to
//      rethrow (webhook-invoked paths) or suppress (cron-invoked paths).
//
// FUTURE OPTIMIZATION: The current 5-action allow-list has deterministic
// triggers (401 → refresh_dropbox_token, 5xx → retry, etc.) that don't require
// an LLM. A hybrid design — deterministic heuristic first, Claude only when
// the heuristic returns unknown — would be faster, cheaper, and more reliable.
// The LLM earns its value on novel errors outside the regex table. Refactor
// to hybrid when the action set grows beyond the initial five.
// See adversarial review finding from Session 9.
...
// Best-effort redaction, not a security boundary. Only checks key names
// against a narrow regex — misses `authorization`, `Cookie`, `x-api-key`,
// and any secret embedded inside an innocuously-named value. Do not rely
// on this to keep credentials out of prompts; treat all context passed to
// self-heal as potentially visible to Claude.
...
// TODO (#3 from Session 9 adversarial review): the dedup key (agent, action)
// is too coarse — every unhandled_rejection dedups against every other one,
// and every handleStatusChange dedups regardless of taskId. Include a short
// hash of the error message in the key so unrelated failures aren't silenced.
//
// TODO (#7 from Session 9 adversarial review): filter this query to
// `status != 'success'` so a successful recovery in the window doesn't block
// diagnosis of a new, different failure. Today any attempt (success or
// failure) exhausts the window.
async function wasRecentlyAttempted(context) {
  try {
    ...
    if (error) {
      // TODO (#4 from Session 9 adversarial review): fail-open tradeoff.
      // Failing open means that when Supabase is flaky, the dedup guard
      // disengages exactly when error volume is spiking, risking retry
      // storms. Failing closed would instead mean zero recovery during a
      // Supabase outage, which could mask recoverable errors (e.g., a
      // Dropbox 401 that refresh_dropbox_token would handle cleanly).
      // Kept as fail-open for now — revisit if we see storm behavior.
 ...
 *
 * @param {Error} error
 * @param {object} context - { agent, action, taskId?, videoId?, campusId?, payload?, retryFn? }
 * @param {object} [options]
 * @param {Function} [options.retry] - async () => <result>. If Claude recommends retry, this is invoked once after a 2s delay. Takes precedence over context.retryFn.
 * @returns {Promise<{ recovered: boolean }>} — callers decide whether to rethrow based on this. Webhook-invoked paths should rethrow when recovered === false so the webhook_inbox row gets marked failed for replay; cron-invoked paths typically swallow either way.
 */
async function handle(error, context, options = {}) {
  try {
    ...
    // Resolve retry callback. options.retry (explicit, passed at call site)
    // takes precedence over context.retryFn (default for the agent). Cron
    // agents wire retryFn on context; pipeline/server pass nothing.
    const retryCallback = options.retry || context.retryFn || null;
    ...
    let attempt;
    try {
      await fn({ retry: retryCallback, params: diagnosis.recovery_params, context });
